...
# Installation
Depending on the deployment environment, it may make sense to create a conda or virtualenv. If doing so, ensure that the python version is >=3.10,<3.14.

```
conda create -n sequencing-runs-collector python=3.13 pip
...
```json
{
    ...
    "collect_fastq_stats": true,
    "num_fastq_stats_collection_processes": 16,
...
`translated_project_id`

For example:

```csv
samplesheet_project_id,translated_project_id
